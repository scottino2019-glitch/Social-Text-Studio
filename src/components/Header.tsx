...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header
      id="app-header"
      className="h-16 bg-white border-b-4 border-black px-4 sm:px-6 flex items-center justify-between z-30 sticky top-0 shadow-sm"
    >
      {/* Brand & Title */}
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-[#FF3366] border-2 border-black rounded-lg flex items-center justify-center shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-white font-black text-xl tracking-tight">
          T!
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="hidden sm:inline-flex text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-[#FFD700] text-black border-2 border-black font-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
            Studio
          </span>
        </div>
      </div>

      {/* Center Tools: Format Selector & Undo/Redo */}
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div ref={dropdownRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 bg-white hover:bg-neutral-50 border-2 border-black rounded-xl px-3 py-1.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-xs font-black transition-all cursor-pointer"
          >
            <div className="p-1 rounded bg-[#FDFCF5] border border-black flex items-center justify-center">
              {getFormatIcon(currentFormat)}
            </div>
            <span className="text-black font-black">{currentFormat.name.split(' ')[0]}</span>
          ...
          {isFormatDropdownOpen && (
            <div className="absolute top-full mt-2 left-0 w-64 bg-white border-3 border-black rounded-2xl p-2 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] z-50 flex flex-col gap-1.5">
        ...
        {/* Undo / Redo */}
        <div className="hidden sm:flex items-center bg-white border-2 border-black rounded-xl p-1 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
          <button
            id="undo-action-btn"
            onClick={onUndo}
            disabled={!canUndo}
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-xl border-2 border-black text-xs font-black flex items-center gap-1.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all cursor-pointer ${
            isPlayingAnimation
        ...
        >
          {isPlayingAnimation ? (
            <>
              ...
              <span className="hidden lg:inline">ANIMAZIONE LIVE</span>
            </>
          ) : (
            <>
      ...
      <div className="flex items-center gap-2">
        <button
          id="add-text-btn"
          onClick={onAddText}
          className="px-3.5 py-2 rounded-full bg-white hover:bg-neutral-100 text-black text-xs font-black flex items-center gap-1.5 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all cursor-pointer"
        >
        ...
        <button
          ...
          className="px-5 py-2 rounded-full bg-[#33FFBB] hover:bg-[#28e6a5] text-black text-xs font-black tracking-tight flex items-center gap-2 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all disabled:opacity-50 cursor-pointer"
        >
          <Download className="w-4 h-4 stroke-[2.5]" />
          <span>ESPORTA HD</span>
        </button>
      </div>
    </header>
  );
